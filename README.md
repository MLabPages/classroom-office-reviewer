...
操作パネルの「Google形式はPDFにせず表示」をオンにすると、Googleドキュメント／スライドはPDFへ変換せず、Classroomの表示のまま一覧へ加えます。変換によるレイアウトのずれが起きず、表示も速くなります。PDFビューアの操作（全画面・ページ送りなど）を使いたい場合はオフのままにしてください。既定はオフ（PDFへ変換）です。
