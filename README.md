...
採点や授業の前に「全員分を一括準備」を押すと、準備専用タブを1枚だけ開き、そこで先頭の提出者から順番にWord／PowerPointの取得・PDF化と、Googleドキュメント／スライドのPDF取得を行います。既にPDFなど対象外の提出物は、保存画面を開かず読み飛ばします。準備専用タブは処理のたびに増減せず、完了後も閉じません。元の採点タブはそのまま使用でき、進行状況も右下の小さなパネルで確認できます。準備専用タブは自動で切り替わるため、完了まで操作しないでください。「次の提出物を自動表示」をオンにすると、準備済みPDFへ自動で切り替わります。
