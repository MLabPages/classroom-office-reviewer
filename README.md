...
全画面中は、矢印キー、PageUp／PageDown、Spaceでページを送り、Escで終了します。「次の提出物も自動表示」をオンにすると、学生の切替後も同じ表示方法を続けます。次のファイルのPDF化は裏で進み、変換が終わるまでは前の提出物を消さずに表示します。通常表示も採点画面の大半を使う大きさになり、必要なら「全画面」と「＋」でさらに拡大できます。
