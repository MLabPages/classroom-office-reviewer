...
採点や授業の前に「全員分を一括準備」を押すと、同じClassroomタブで先頭の提出者から順番に移動し、Word／PowerPointの取得とPDF化をまとめて行います。別タブは開きません。進行中は準備済み件数と現在の処理段階を表示し、完了後は先頭の提出者へ戻ります。「次の提出物を自動表示」をオンにすると、準備済みPDFへ自動で切り替わります。
