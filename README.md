...
操作パネルの「Google形式はPDFにせず表示」をオンにすると、Googleドキュメント／スライドはPDFへ変換せず、Googleの表示をそのままビューア内に大きく出します。変換によるレイアウトのずれが起きず、表示も速くなります。PDFのページ送りや倍率変更を使いたい場合はオフのままにしてください。既定はオフ（PDFへ変換）です。

### 全画面表示の操作

全画面中はマウスを画面の上端へ近づけると操作欄が出て、倍率の変更・ページ送り・提出物一覧をそのまま使えます。キーボードでも操作できます。

| キー | 動作 |
|---|---|
| →／PageDown／Space | 次のページへ |
| ←／PageUp | 前のページへ |
| ＋／− | 拡大／縮小 |
| F | 全画面の切り替え |
| Esc | 全画面を終了 |
